...
// hooks
add_action('ppqtranslate_css',					'qs_css');
add_action('qs_cron_hook',						'qs_cron');
add_action('ppqtranslate_configuration',			'qs_config_hook');
add_action('ppqtranslate_loadConfig',				'qs_load');
add_action('ppqtranslate_saveConfig',				'qs_save');
add_action('ppqtranslate_clean_uri',				'qs_clean_uri');
add_action('admin_menu',						'qs_init');
add_action('wp_ajax_qs_quote', 					'qs_quote');

add_filter('manage_order_columns',				'qs_order_columns');
add_filter('ppqtranslate_configuration_pre',		'qs_config_pre_hook');
...
function qs_translateButtons($available_languages, $missing_languages) {
	...
	foreach($missing_languages as $language) {
		$missing_languages_name[] = '<a href="edit.php?page=ppqtranslate_services&post='.$post->ID.'&target_language='.$language.'">'.$q_config['language_name'][$language].'</a>';
	}
...
#ppqtrans_select_translate { margin-right:11px }
.qs_status { border:0 }
...
function qs_init() {
	global $q_config;
	if($q_config['ppqtranslate_services']) {
	/* disabled for meta box
		add_filter('ppqtranslate_toolbar',			'qs_toobar');
		add_filter('ppqtranslate_modify_editor_js',	'qs_editor_js');
	*/
		...
		add_action('ppqtranslate_languageColumn',			'qs_translateButtons', 10, 2);
		
		// add plugin page without menu link for users with permission
		if(current_user_can('edit_published_posts')) {
			//add_posts_page(__('Translate','qtranslate'), __('Translate','qtranslate'), 'edit_published_posts', 'ppqtranslate_services', 'qs_service');
			global $_registered_pages;
			$hookname = get_plugin_page_hookname('ppqtranslate_services', 'edit.php');
...
function qs_save() {
	global $q_config;
	if($q_config['ppqtranslate_services'])
		update_option('ppqtranslate_ppqtranslate_services', '1');
	else
		update_option('ppqtranslate_ppqtranslate_services', '0');
}
...
function qs_config_pre_hook($message) {
	...
	if(isset($_POST['default_language'])) {
		ppqtrans_checkSetting('ppqtranslate_services', true, QT_BOOLEAN);
		qs_load();
		if($q_config['ppqtranslate_services']) {
...
function qs_config_hook($request_uri) {
	global $q_config;
?>
<h3><?php _e('qTranslate Services Settings', 'qtranslate') ?><span id="qtranslate-show-services" style="display:none"> (<a name="ppqtranslate_service_settings" href="#ppqtranslate_service_settings" onclick="showServices();"><?php _e('Show', 'qtranslate'); ?></a>)</span></h3>
<table class="form-table" id="qtranslate-services">
	<tr>
		...
		<td>
			...
			<label for="ppqtranslate_services"><input type="checkbox" name="ppqtranslate_services" id="ppqtranslate_services" value="1"<?php echo ($q_config['ppqtranslate_services'])?' checked="checked"':''; ?>/> <?php _e('Enable qTranslate Services', 'qtranslate'); ?></label>
			<br/>
...
<?php
		foreach($orders as $order) { 
			...
			if(!$post) continue;
			$post->post_title = esc_html(ppqtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($post->post_title));
?>
				<tr>
					...
					<td class="qs_no-bottom-border"><a class="delete" href="<?php echo add_query_arg('qs_delete', $order['order']['order_id'], $request_uri); ?>#ppqtranslate_service_settings">Delete</a></td>
				</tr>
<?php 
...
?>
			...
			<p><?php printf(__('qTranslate Services will automatically check every hour whether the translations are finished and update your posts accordingly. You can always <a href="%s">check manually</a>.','qtranslate'),'options-general.php?page=qtranslate&qs_cron=true#ppqtranslate_service_settings'); ?></p>
...
// <![CDATA[
	...
	if(location.hash!='#ppqtranslate_service_settings') {
...
function qs_UpdateOrder($order_id) {
	...
	foreach($orders as $key => $order) {
		...
		if(isset($result['order_status']) && $result['order_status']==QS_STATE_CLOSED) {
			...
			$post = &get_post($order['post_id']);
			$title = ppqtrans_split($post->post_title);
			$content = ppqtrans_split($post->post_content);
			$title[$order['target_language']] = $result['order_translated_title'];
			$content[$order['target_language']] = $result['order_translated_text'];
			$post->post_title = ppqtrans_join($title);
			$post->post_content = ppqtrans_join($content);
			$wpdb->show_errors();
...
<ul>
	...
	<li><a href="options-general.php?page=qtranslate#ppqtranslate_service_settings"><?php _e('View all open orders.', 'qtranslate'); ?></a></li>
	<li><a href="options-general.php?page=qtranslate&qs_cron=true#ppqtranslate_service_settings"><?php _e('Let qTranslate Services check if any open orders are finished.', 'qtranslate'); ?></a></li>
...
?>
<li>
	<label><input type="radio" name="service_id" disabled="disabled" /> <b><?php echo ppqtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($service['service_name']); ?></b> ( <a href="<?php echo $service['service_url']; ?>" target="_blank"><?php _e('Website', 'qtranslate'); ?></a> )</label>
...
function qs_quote() {
	...
	$request = array(
			...
			'order_target_language' => $translate_to,
			'order_target_locale' => $q_config['locale'][$translate_to],
			'order_confirm_url' => get_admin_url(null, 'edit.php?page=ppqtranslate_services&confirm=1&post='.$_POST['post_id'].'&source_language='.$translate_from.'&target_language='.$translate_to.'&service_id='.$service_id),
			'order_failure_url' => get_admin_url(null, 'edit.php?page=ppqtranslate_services&post='.$_POST['post_id'].'&source_language='.$translate_from.'&target_language='.$translate_to.'&service_id='.$service_id)
		);
...
function qs_editor_js($content) {
	$content .= "
		init_qs = function(action, id) {
			document.location.href = 'edit.php?page=ppqtranslate_services&post=".intval($_REQUEST['post'])."';
		}
		";
	return $content;
}
