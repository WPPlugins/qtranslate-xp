...
function ppqtrans_init() {
	global $q_config;
	// check if it isn't already initialized
	if(defined('ppqtrans_INIT')) return;
	define('ppqtrans_INIT',true);
	// load configuration if not beeing reseted
	if(isset($_POST['ppqtranslate_reset']) && isset($_POST['ppqtranslate_reset2']) && defined('WP_ADMIN') && current_user_can('manage_options')) {
		// reset all settings
		delete_option('ppqtranslate_language_names');
		delete_option('ppqtranslate_enabled_languages');
		delete_option('ppqtranslate_default_language');
		delete_option('ppqtranslate_flag_location');
		delete_option('ppqtranslate_flags');
		delete_option('ppqtranslate_locales');
		delete_option('ppqtranslate_na_messages');
		delete_option('ppqtranslate_date_formats');
		delete_option('ppqtranslate_time_formats');
		delete_option('ppqtranslate_use_strftime');
		delete_option('ppqtranslate_ignore_file_types');
		delete_option('ppqtranslate_url_mode');
		delete_option('ppqtranslate_detect_browser_language');
		delete_option('ppqtranslate_hide_untranslated');
		delete_option('ppqtranslate_auto_update_mo');
		delete_option('ppqtranslate_next_update_mo');
		delete_option('ppqtranslate_hide_default_language');
		if(isset($_POST['ppqtranslate_reset3'])) {
			delete_option('ppqtranslate_term_name');
		}
	}
	ppqtrans_loadConfig();
	if(isset($_COOKIE['ppqtrans_cookie_test'])) {
	...
	if(defined('WP_ADMIN') && $q_config['auto_update_mo']) ppqtrans_updateGettextDatabases();
	
	// update definitions if neccesary
	if(defined('WP_ADMIN') && current_user_can('manage_categories')) ppqtrans_updateTermLibrary();
	
	// extract url information
	$q_config['url_info'] = ppqtrans_extractURL($_SERVER['REQUEST_URI'], $_SERVER["HTTP_HOST"], isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '');
	
	// set test cookie
	setcookie('ppqtrans_cookie_test', 'qTranslate Cookie Test', 0, $q_config['url_info']['home'], $q_config['url_info']['host']);
	// check cookies for admin
	if(defined('WP_ADMIN')) {
		if(isset($_GET['lang']) && ppqtrans_isEnabled($_GET['lang'])) {
			$q_config['language'] = $q_config['url_info']['language'];
			setcookie('ppqtrans_admin_language', $q_config['language'], time()+60*60*24*30);
		} elseif(isset($_COOKIE['ppqtrans_admin_language']) && ppqtrans_isEnabled($_COOKIE['ppqtrans_admin_language'])) {
			$q_config['language'] = $_COOKIE['ppqtrans_admin_language'];
		} else {
	...
	$q_config['language'] = apply_filters('ppqtranslate_language', $q_config['language']);
	
	// detect language and forward if needed
	if($q_config['detect_browser_language'] && $q_config['url_info']['redirect'] && !isset($_COOKIE['ppqtrans_cookie_test']) && $q_config['url_info']['language'] == $q_config['default_language']) {
		$target = false;
		...
		if(isset($_SERVER["HTTP_ACCEPT_LANGUAGE"]) && preg_match_all("#([^;,]+)(;[^,0-9]*([0-9\.]+)[^,]*)?#i",$_SERVER["HTTP_ACCEPT_LANGUAGE"], $matches, PREG_SET_ORDER)) {
			...
			foreach($prefered_languages as $language => $priority) {
				...
				if(ppqtrans_isEnabled($language)) {
					if($q_config['hide_default_language'] && $language == $q_config['default_language']) break;
					$target = ppqtrans_convertURL(get_option('home'),$language);
					break;
				}
			}
		}
		$target = apply_filters("ppqtranslate_language_detect_redirect", $target);
		if($target !== false) {
			wp_redirect($target);
			exit();
		}
	...
	*/
	...
	if(!defined('WP_ADMIN')) {
		...
		foreach($alloptions as $option => $value) {
			add_filter('option_'.$option, 'ppqtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage',0);
		}
	}
	...
	$q_config['url_info']['url'] = ppqtrans_convertURL(add_query_arg('lang',$q_config['default_language'],$q_config['url_info']['url']));
}

// returns cleaned string and language information
function ppqtrans_extractURL($url, $host = '', $referer = '') {
	global $q_config;
	$home = ppqtrans_parseURL(get_option('home'));
	$home['path'] = trailingslashit($home['path']);
	$referer = ppqtrans_parseURL($referer);
	...
	if($referer['host']==$result['host'] && ppqtrans_startsWith($referer['path'], $home['path'])) {
...
function ppqtrans_validateBool($var, $default) {
...
function ppqtrans_saveConfig() {
	global $q_config;
	
	// save everything
	update_option('ppqtranslate_language_names', $q_config['language_name']);
	update_option('ppqtranslate_enabled_languages', $q_config['enabled_languages']);
	update_option('ppqtranslate_default_language', $q_config['default_language']);
	update_option('ppqtranslate_flag_location', $q_config['flag_location']);
	update_option('ppqtranslate_flags', $q_config['flag']);
	update_option('ppqtranslate_locales', $q_config['locale']);
	update_option('ppqtranslate_na_messages', $q_config['not_available']);
	update_option('ppqtranslate_date_formats', $q_config['date_format']);
	update_option('ppqtranslate_time_formats', $q_config['time_format']);
	update_option('ppqtranslate_ignore_file_types', $q_config['ignore_file_types']);
	update_option('ppqtranslate_url_mode', $q_config['url_mode']);
	update_option('ppqtranslate_term_name', $q_config['term_name']);
	update_option('ppqtranslate_use_strftime', $q_config['use_strftime']);
	if($q_config['detect_browser_language'])
		update_option('ppqtranslate_detect_browser_language', '1');
	else
		update_option('ppqtranslate_detect_browser_language', '0');
	if($q_config['hide_untranslated'])
		update_option('ppqtranslate_hide_untranslated', '1');
	else
		update_option('ppqtranslate_hide_untranslated', '0');
	if($q_config['auto_update_mo'])
		update_option('ppqtranslate_auto_update_mo', '1');
	else
		update_option('ppqtranslate_auto_update_mo', '0');
	if($q_config['hide_default_language'])
		update_option('ppqtranslate_hide_default_language', '1');
	else
		update_option('ppqtranslate_hide_default_language', '0');
		
	do_action('ppqtranslate_saveConfig');
}

function ppqtrans_updateGettextDatabases($force = false, $only_for_language = '') {
	global $q_config;
	if(!is_dir(WP_LANG_DIR)) {
		if(!@mkdir(WP_LANG_DIR))
			return false;
	}
	$next_update = get_option('ppqtranslate_next_update_mo');
	if(time() < $next_update && !$force) return true;
	update_option('ppqtranslate_next_update_mo', time() + 7*24*60*60);
	foreach($q_config['locale'] as $lang => $locale) {
		if(ppqtrans_isEnabled($only_for_language) && $lang != $only_for_language) continue;
		if(!ppqtrans_isEnabled($lang)) continue;
...
function ppqtrans_updateTermLibrary() {
	global $q_config;
	...
	switch($_POST['action']) {
		...
		case 'add-tag':
		case 'add-link-cat':
			if(isset($_POST['ppqtrans_term_'.$q_config['default_language']]) && $_POST['ppqtrans_term_'.$q_config['default_language']]!='') {
				$default = htmlspecialchars(ppqtrans_stripSlashesIfNecessary($_POST['ppqtrans_term_'.$q_config['default_language']]), ENT_NOQUOTES);
				if(!isset($q_config['term_name'][$default]) || !is_array($q_config['term_name'][$default])) $q_config['term_name'][$default] = array();
				foreach($q_config['enabled_languages'] as $lang) {
					$_POST['ppqtrans_term_'.$lang] = ppqtrans_stripSlashesIfNecessary($_POST['ppqtrans_term_'.$lang]);
					if($_POST['ppqtrans_term_'.$lang]!='') {
						$q_config['term_name'][$default][$lang] = htmlspecialchars($_POST['ppqtrans_term_'.$lang], ENT_NOQUOTES);
					} else {
...
function ppqtrans_strftime($format, $date, $default = '', $before = '', $after = '') {
...
function ppqtrans_dateFromPostForCurrentLanguage($old_date, $format ='', $before = '', $after = '') {
	global $post;
	return ppqtrans_strftime(ppqtrans_convertDateFormat($format), mysql2date('U',$post->post_date), $old_date, $before, $after);
}

function ppqtrans_dateModifiedFromPostForCurrentLanguage($old_date, $format ='') {
	global $post;
	return ppqtrans_strftime(ppqtrans_convertDateFormat($format), mysql2date('U',$post->post_modified), $old_date);
}

function ppqtrans_timeFromPostForCurrentLanguage($old_date, $format = '', $post = null, $gmt = false) {
	$post = get_post($post);
	
	$post_date = $gmt? $post->post_date_gmt : $post->post_date;
	return ppqtrans_strftime(ppqtrans_convertTimeFormat($format), mysql2date('U',$post_date), $old_date);
}

function ppqtrans_timeModifiedFromPostForCurrentLanguage($old_date, $format = '', $gmt = false) {
	global $post;
	$post_date = $gmt? $post->post_modified_gmt : $post->post_modified;
	return ppqtrans_strftime(ppqtrans_convertTimeFormat($format), mysql2date('U',$post_date), $old_date);
}

function ppqtrans_dateFromCommentForCurrentLanguage($old_date, $format ='') {
	global $comment;
	return ppqtrans_strftime(ppqtrans_convertDateFormat($format), mysql2date('U',$comment->comment_date), $old_date);
}
...
function ppqtrans_useTermLib($obj) {
	global $q_config;
	if(is_array($obj)) {
		// handle arrays recursively
		foreach($obj as $key => $t) {
			$obj[$key] = ppqtrans_useTermLib($obj[$key]);
		}
		return $obj;
	}
	if(is_object($obj)) {
...
function ppqtrans_convertBlogInfoURL($url, $what) {
	...
	return ppqtrans_convertURL($url);
}

function ppqtrans_convertURL($url='', $lang='', $forceadmin = false) {
	global $q_config;
	...
	if(!ppqtrans_isEnabled($lang)) return "";
	...
	if($urlinfo['host']!='') {
		...
		$homeinfo = ppqtrans_parseURL($home);
	...
	if(preg_match("#(&|\?)lang=([^&\#]+)#i",$url,$match) && ppqtrans_isEnabled($match[2])) {
...
function ppqtrans_split($text, $quicktags = true) {
	global $q_config;
	...
	foreach($blocks as $block) {
		...
		if(preg_match("#^<!--:([a-z]{2})-->$#ism", $block, $matches)) {
			if(ppqtrans_isEnabled($matches[1])) {
...
function ppqtrans_join($texts) {
	global $q_config;
	if(!is_array($texts)) $texts = ppqtrans_split($texts, false);
...
function ppqtrans_disableLanguage($lang) {
	global $q_config;
...
function ppqtrans_enableLanguage($lang) {
	global $q_config;
	if(ppqtrans_isEnabled($lang) || !isset($q_config['language_name'][$lang])) {
		return false;
	}
	$q_config['enabled_languages'][] = $lang;
	// force update of .mo files
	if ($q_config['auto_update_mo']) ppqtrans_updateGettextDatabases(true, $lang);
	return true;
}
...
function ppqtrans_use($lang, $text, $show_available=false) {
	...
	if(!ppqtrans_isEnabled($lang)) return $text;
	if(is_array($text)) {
		// handle arrays recursively
