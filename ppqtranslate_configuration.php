...
function ppqtrans_adminMenu() {
	global $menu, $submenu, $q_config;
	
	/* Configuration Page */
	add_options_page(__('Language Management', 'qtranslate'), __('Languages', 'qtranslate'), 'manage_options', 'qtranslate', 'ppqtranslate_conf');
...
function ppqtranslate_language_form($lang = '', $language_code = '', $language_name = '', $language_locale = '', $language_date_format = '', $language_time_format = '', $language_flag ='', $language_na_message = '', $language_default = '', $original_lang='') {
	global $q_config;
?>
...
function ppqtrans_checkSetting($var, $updateOption = false, $type = QT_STRING) {
	global $q_config;
	switch($type) {
		case QT_URL:
		...
		case QT_STRING:
			if(isset($_POST['submit']) && isset($_POST[$var])) {
				if($type != QT_LANGUAGE || ppqtrans_isEnabled($_POST[$var])) {
					$q_config[$var] = $_POST[$var];
				}
				if($updateOption) {
					update_option('ppqtranslate_'.$var, $q_config[$var]);
				}
				return true;
			} else {
				return false;
		...
		case QT_BOOLEAN:
			if(isset($_POST['submit'])) {
				...
				if($updateOption) {
					if($q_config[$var]) {
						update_option('ppqtranslate_'.$var, '1');
					} else {
						update_option('ppqtranslate_'.$var, '0');
					}
				}
				return true;
			} else {
...
function ppqtrans_language_columns($columns) {
	return array(
				'flag' => 'Flag',
...
function ppqtranslate_conf() {
	global $q_config, $wpdb;
	
	// do redirection for dashboard
	if(isset($_GET['godashboard'])) {
		echo '<h2>'.__('Switching Language', 'qtranslate').'</h2>'.sprintf(__('Switching language to %1$s... If the Dashboard isn\'t loading, use this <a href="%2$s" title="Dashboard">link</a>.','qtranslate'),$q_config['language_name'][ppqtrans_getLanguage()],admin_url()).'<script type="text/javascript">document.location="'.admin_url().'";</script>';
		exit();
	}
	...
	$message = apply_filters('ppqtranslate_configuration_pre','');
	
	// check for action
	if(isset($_POST['ppqtranslate_reset']) && isset($_POST['ppqtranslate_reset2'])) {
		$message = __('qTranslate has been reset.', 'qtranslate');
	} elseif(isset($_POST['default_language'])) {
		// save settings
		ppqtrans_checkSetting('default_language',			true, QT_LANGUAGE);
		ppqtrans_checkSetting('flag_location',			true, QT_URL);
		ppqtrans_checkSetting('ignore_file_types',		true, QT_STRING);
		ppqtrans_checkSetting('detect_browser_language',	true, QT_BOOLEAN);
		ppqtrans_checkSetting('hide_untranslated',		true, QT_BOOLEAN);
		ppqtrans_checkSetting('use_strftime',				true, QT_INTEGER);
		ppqtrans_checkSetting('url_mode',					true, QT_INTEGER);
		ppqtrans_checkSetting('auto_update_mo',			true, QT_BOOLEAN);
		ppqtrans_checkSetting('hide_default_language',	true, QT_BOOLEAN);
		if(isset($_POST['update_mo_now']) && $_POST['update_mo_now']=='1' && ppqtrans_updateGettextDatabases(true))
	...
	if(isset($_POST['original_lang'])) {
		...
		foreach($result as $post) {
			...
			$content = ppqtrans_join($content);
			$title = ppqtrans_join($title);
		...
		if($error=='') {
			...
			ppqtrans_disableLanguage($_GET['delete']);
		...
		// enable validate
		if(!ppqtrans_enableLanguage($_GET['enable'])) {
	...
	$clean_uri = apply_filters('ppqtranslate_clean_uri', $clean_uri);
...
<?php ppqtranslate_language_form($language_code, $language_code, $language_name, $language_locale, $language_date_format, $language_time_format, $language_flag, $language_na_message, $language_default, $original_lang); ?>
	...
	<form action="<?php echo $clean_uri;?>" method="post">
		...
		<table class="form-table">
			<tr>
				...
				<?php
					foreach ( ppqtrans_getSortedLanguages() as $key => $language ) {
		...
		<table class="form-table" id="qtranslate-advanced">
			...
			<tr valign="top">
				...
				<td>
					<label for="ppqtranslate_reset"><input type="checkbox" name="ppqtranslate_reset" id="ppqtranslate_reset" value="1"/> <?php _e('Check this box and click Save Changes to reset all qTranslate settings.', 'qtranslate'); ?></label>
					<br/>
					<label for="ppqtranslate_reset2"><input type="checkbox" name="ppqtranslate_reset2" id="ppqtranslate_reset2" value="1"/> <?php _e('Yes, I really want to reset qTranslate.', 'qtranslate'); ?></label>
					<br/>
					<label for="ppqtranslate_reset3"><input type="checkbox" name="ppqtranslate_reset3" id="ppqtranslate_reset3" value="1"/> <?php _e('Also delete Translations for Categories/Tags/Link Categories.', 'qtranslate'); ?></label>
					<br/>
			...
			<tr>
				...
				<td>
					...
					<textarea readonly="readonly" id="ppqtranslate_debug"><?php
...
<?php do_action('ppqtranslate_configuration', $clean_uri); ?>
		<p class="submit">
...
<?php ppqtranslate_language_form($language_code, $language_code, $language_name, $language_locale, $language_date_format, $language_time_format, $language_flag, $language_default, $language_na_message); ?>
